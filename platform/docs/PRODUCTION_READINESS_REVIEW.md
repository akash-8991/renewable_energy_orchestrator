...
| Cloud deployment | Terraform written (`infra/terraform/*.tf` — VPC, ECS Fargate, RDS, ElastiCache, S3, Secrets Manager, ALB), `terraform validate`-clean, **never `apply`d**. | An actual AWS account, a real `terraform apply`, then load testing against that environment (nothing here has been tested under concurrent load — the whole verification history is single-tenant, low-QPS). |
...
1. **AWS apply** — provision the already-`validate`-clean Terraform for real, then load-test it.
