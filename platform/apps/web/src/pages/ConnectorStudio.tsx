...
interface Connector {
  id: string; name: string; endpoint_url: string; method: string; status: string;
...
export default function ConnectorStudio() {
    ...
    mutationFn: async () =>
      (
        await api.post("/connectors", {
          name, endpoint_url: url, method: "GET",
  ...
  return (
    <div>
      ...
      {data && (
        <table>
          <thead><tr><th>Name</th><th>Endpoint</th><th>Status</th><th>Credential</th><th>Last test</th><th></th></tr></thead>
          <tbody>
            {data.map((c) => (
              <tr key={c.id}>
                <td>{c.name}</td>
